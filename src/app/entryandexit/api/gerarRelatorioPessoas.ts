...
export async function gerarRelatorioPessoasPdf({
  ...
  // Brasão
  const imageBytes = await fetch('/brasao.png').then((res) => res.arrayBuffer());
  ...
  page.drawText('SECRETARIA DA SEGURANÇA PÚBLICA', { x: 150, y, size: 14, font: boldFont });
  page.drawText('POLÍCIA MILITAR DO ESTADO DE SÃO PAULO', { x: 150, y: y - 20, size: 14, font: boldFont });
  page.drawText(`Carapicuíba, ${formatDate(dataLista)}.`, { x: 150, y: y - 40, size: 12, font });
  page.drawText(`PARTE N.º ${numeroParte}`, { x: 150, y: y - 60, size: 12, font });
  page.drawText(`DO ${nomeGuerra}`, { x: 150, y: y - 80, size: 12, font });
  page.drawText(`Ao Sr Oficial de Semana.`, { x: 150, y: y - 100, size: 12, font });
  page.drawText(`Assunto: Relação de Entrada e Saída de Pessoas.`, { x: 150, y: y - 120, size: 12, font });
  ...
  // Bloco Oficial
  page.drawText(`Em _____/_____/2025.`, { x: marginLeft, y: y - 30, size: 12, font });
  page.drawText(`__________________`, { x: marginLeft, y: y - 50, size: 12, font });
  page.drawText(`Do Oficial de Semana`, { x: marginLeft, y: y - 70, size: 12, font });
  page.drawText(`Ao Sr. ____________`, { x: marginLeft, y: y - 90, size: 12, font });
  page.drawText(`__________________`, { x: marginLeft, y: y - 110, size: 12, font });

  // Bloco Sgt
  page.drawText(`Em _____/_____/2025.`, { x: marginLeft + 150, y: y - 30, size: 12, font });
  page.drawText(`__________________`, { x: marginLeft + 150, y: y - 50, size: 12, font });
  page.drawText(`Do Sgt de Semana`, { x: marginLeft + 150, y: y - 70, size: 12, font });
  page.drawText(`Ao Sr. Oficial de Semana`, { x: marginLeft + 150, y: y - 90, size: 12, font });
  page.drawText(`__________________`, { x: marginLeft + 150, y: y - 110, size: 12, font });
  ...
  const assinaturaTexto = `${abreviarPatente(patenteCompleta)} ${reResponsavel} ${nome}`;
  ...
  const assinaturaTextoX = linhaAssinaturaX + (linhaAssinaturaWidth - assinaturaWidth) / 2;
...
function desenharRodapeEmTodasPaginas({
  ...
  const frase = '"Nós Policiais Militares, sob a proteção de Deus, estamos compromissados com a defesa da Vida, da Integridade Física e Dignidade Humana."';
